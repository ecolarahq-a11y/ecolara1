...
CREATE OR REPLACE FUNCTION public.submit_quiz_result(p_module_id integer, p_correct integer, p_total integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  v_best := GREATEST(v_prev, v_pct);
  v_scores := v_scores || jsonb_build_object(p_module_id::text, v_best);

  -- Idempotent point awarding: only award the delta between new best and previous best
  v_prev_points := CASE
    WHEN v_prev = 0 THEN 0
    WHEN v_prev <= 30 THEN 5
    WHEN v_prev <= 50 THEN 10
    WHEN v_prev <= 70 THEN 20
    WHEN v_prev <= 80 THEN 30
    ELSE 50
  END;
  IF v_best > v_prev THEN
    v_points_awarded := v_points - v_prev_points;
    IF v_points_awarded < 0 THEN v_points_awarded := 0; END IF;
  ELSE
    v_points_awarded := 0;
  END IF;
  ...
  v_prog.total_points := v_prog.total_points + v_points_awarded;
  v_level := CASE
